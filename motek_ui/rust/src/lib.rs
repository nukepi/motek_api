...
pub fn init_logging() {
    INIT.call_once(|| {
        // Utworzenie ścieżki do pliku logów
        let log_path = std::env::temp_dir().join("motek_ui.log");
        let log_path_str = log_path.to_string_lossy();
        
        // Utworzenie zapisywacza plików
        ...
        // Utworzenie zapisywacza dla konsoli
        let (non_blocking, _guard) = tracing_appender::non_blocking(std::io::stdout());

        // Konfiguracja subskrybenta z filtrem środowiskowym
        ...
        // Ustawienie subskrybenta jako globalnego
